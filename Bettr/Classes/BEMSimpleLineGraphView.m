...
#pragma clang diagnostic pop
    ...
    if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
        ...
        if (self.enablePopUpReport == YES) {
            self.popUpLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 120, 20)];
            float newNum = [[self calculateMaximumPointValue] floatValue] * 0.08;
            self.popUpLabel.text = [NSString stringWithFormat:@"%.0f", newNum];
            ...
            self.popUpView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.popUpLabel.frame.size.width + 3, self.popUpLabel.frame.size.height + 2)];
...
- (void)setUpPopUpLabelAbovePoint:(BEMCircle *)closestPoint {
    ...
    float newNum = [[dataPoints objectAtIndex:(NSInteger)closestDot.tag - 100] floatValue] * 0.08;
    self.popUpLabel.text = [NSString stringWithFormat:@"%.0f points", newNum];
